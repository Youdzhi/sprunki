		...
		if (entity.isShiftKeyDown()) {
			...
			SprunikiMod.queueServerWork(30, () -> {
				for (int index0 = 0; index0 < 12; index0++) {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = SprunikiModEntities.FARO_7.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
				...
				SprunikiMod.queueServerWork(10, () -> {
					{
						...
						if (!_ent.level().isClientSide() && _ent.getServer() != null) {
							_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
									_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "spreadplayers ~ ~ 1 16 false @e[type=spruniki:faro_7]");
						}
					}
				});
			});
